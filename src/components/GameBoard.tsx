...
export function GameBoard({ game, muted, onSaveCloud, canSaveCloud }: GameBoardProps) {
  ...
  function tileStyle(tier: number, ghost = false): CSSProperties {
    const tierGrowth = Math.min(36, Math.max(0, tier - 1) * 4.5);
    return {
      "--tile-size": `${Math.min(98, 76 + tierGrowth)}%`,
      "--icon-size": `${Math.min(124, 106 + tierGrowth * 0.5)}%`,
      "--ghost-size": `${Math.min(96, 62 + tierGrowth)}px`,
    ...
    <section className={`board-panel board-theme-${game.activeTheme}`} aria-labelledby="board-title">
          ...
          return (
            ...
            >
              {item && tier ? (
                ...
                >
                  <img className="venture-icon" src={tier.icon} alt="" draggable={false} />
      ...
      {drag.dragGhost ? (
        <span
          ...
          aria-hidden="true"
        >
          <img className="venture-icon" src={getTier(drag.dragGhost.item.tier).icon} alt="" draggable={false} />
